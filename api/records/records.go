...
func CreateRelationShips(session core.Neo4jAPI, datasetId int, organizationId int, userId string, q PostRecordRelationshipRequestBody) ([]ShortRecordRelationShip, error) {
	...
	results, err := session.Run(ctx, cql, params)
	if err != nil {
	...
	result, err := session.Run(ctx, cql2, params)
	if err != nil {
	...
	var relationships []ShortRecordRelationShip
	for i, _ := range txRecords {
		relationships = append(relationships, parseResponse(txRecords[i], relType.(string)))
	}
	...
	log.Println("txResults: ", txRecords)
...
func parseResponse(record *db.Record, relType string) ShortRecordRelationShip {
