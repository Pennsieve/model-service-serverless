...
func GetModelByName(session core.Neo4jAPI, modelName string, datasetId int, organizationId int) (*Model, error) {
	var cql strings.Builder

	cql.WriteString(fmt.Sprintf("MATCH  (m:Model{name:'%s'})", modelName))
	...
	m := parseResponse(record)
	return &m, nil

}

// GetModels returns a list of models for a provided dataset within an organization
func GetModels(session core.Neo4jAPI, datasetId int, organizationId int) ([]Model, error) {
	...
	for result.Next(ctx) {
		...
		models = append(models, parseResponse(record))
	}
...
func parseResponse(record *db.Record) Model {
